...
import { ActiveSession, AuthResponse, AuthStatus, LoginFailure, PendingSignOut, RefusalDetail, SessionEndReason, SignInGrant, StoredSession, TokenGrant } from '@app/core/models/auth.model';
...
export class AuthService {
    ...
    login(credentials: { email: string; password: string; remember: boolean }): Observable<void> {
        ...
        return this._http.post<AuthResponse<SignInGrant>>(this.url(APIEndpoint.SIGN_IN), body, { withCredentials: true }).pipe(
            tap((response) => {
    ...
    listSessions(): Observable<ActiveSession[]> {
        return this._http.get<AuthResponse<ActiveSession[]>>(this.url(APIEndpoint.GET_SESSIONS)).pipe(map((response) => response.data ?? []));
    }
    ...
    signOutOtherDevices(): Observable<number> {
        return this._http.post<AuthResponse<{ sessions_closed: number }>>(this.url(APIEndpoint.SIGN_OUT_EVERYWHERE), { keep_current: true }).pipe(map((response) => response.data?.sessions_closed ?? 0));
    }
    ...
    private async requestRenewal(leaveOnRefusal: boolean): Promise<string> {
        ...
        try {
            ...
            const response = await firstValueFrom(this._http.post<AuthResponse<TokenGrant>>(this.url(APIEndpoint.REFRESH_TOKEN), body, { withCredentials: true }));
