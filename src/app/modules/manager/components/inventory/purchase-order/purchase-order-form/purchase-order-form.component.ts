...
export class PurchaseOrderFormComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.drawerForm.controls['warehouse_oid'].valueChanges.subscribe((value: any) => {
    ...
    calculateTotalPriceFromProducts(): void {
        ...
        this.currentTotalPrice = all_products.reduce((acc: number, product: any) => {
    ...
    handleConfirm(): void {
        ...
        this._modal.create({
            ...
            nzFooter: null,
            nzClosable: false,
            nzOnOk: () => this.actionEmitter.emit({ action: 'submit', value: this.form.value }),
        });
    }
    ...
    handleForm(): void {
        if (this.form.valid) {
            if (this.products.length) {
                ...
                this._notificationService.warning('Warning!', 'You have to add at lease 1 or more products!');
            }
        } else {
    ...
    loadSupplierList(): void {
        this._httpService.get(APIEndpoint.GET_SUPPLIER_LIST_FOR_DROPDOWN).subscribe({
            next: (res: any) => {
    ...
    loadProductList(): void {
        this.loading = true;
        this._httpService.get(APIEndpoint.GET_PRODUCT_LIST_FOR_DROPDOWN, { grouped: true }).subscribe({
            next: (res: any) => {
                this.loading = false;
                if (res.body.code === 200) {
                    this.groupedProductList = res.body.data || [];
                    this.productList = this.groupedProductList.flatMap((group: any) => group.options || []);
                }
            },
            error: () => {
                this.loading = false;
    ...
    loadWarehouseList(): void {
        this.loading = true;
        this._httpService.get(APIEndpoint.GET_WAREHOUSE_LIST_FOR_DROPDOWN).subscribe({
            next: (res: any) => {
                this.loading = false;
                if (res.body.code === 200) {
    ...
    loadAisleList(warehouse_oid: any): void {
        this._httpService.get(APIEndpoint.GET_AISLE_LIST_FOR_DROPDOWN, { warehouse_oid }).subscribe({
            next: (res: any) => {
                this.aisleList = [];
                if (res.body.code === 200) {
