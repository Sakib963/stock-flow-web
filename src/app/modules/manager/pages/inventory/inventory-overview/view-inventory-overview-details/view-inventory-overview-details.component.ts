...
@Component({
    ...
    imports: [CommonModule, LoaderComponent, SecondaryButton, AngularSvgIconModule, NgZorroCustomModule, FormsModule, NgxBarcode6Module, PrimaryButton],
...
export class ViewInventoryOverviewDetailsComponent implements OnInit {
    ...
    redirectToProduct(): void {
        this._router.navigate([`/manager/configuration/product/view-product/${this.productDetails.oid}`], { state: { edit: false } });
    }
    ...
    generateBarcode(item: any): void {
        ...
        if (showPrice && !item.selling_price) {
            this._notificationService.warning('Missing Price', 'Cannot generate barcode: Selling price is missing.');
            return;
        }
