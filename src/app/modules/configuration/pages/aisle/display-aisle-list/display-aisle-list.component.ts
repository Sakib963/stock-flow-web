...
import { AISLE_TABLE_CONFIG } from '@app/modules/configuration/config/aisle.table.config';
...
export class DisplayAisleListComponent {
    aisleTableConfig: TableConfig = AISLE_TABLE_CONFIG;
    data: any[] = [];
    totalCount: number = 0;
    ...
    handleListActions(event: any): any {
        if (event.action === 'create') {
            ...
            this.handleViewAisle(event.value.oid);
        } else if (event.action === 'edit') {
            this.handleEditAisle(event.value.oid);
        }
    }
