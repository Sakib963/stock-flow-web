...
describe('CategoryEditComponent', () => {
    ...
    it('asks before saving, naming the category, and saves nothing when the answer is not yet', async () => {
        const fixture = await open();
        await loaded(fixture);
        ...
        expect(asked.mock.calls[0][0]?.nzTitle).toBe('configuration.category.confirmEdit.title');
        TestBed.inject(HttpTestingController).expectNone((r) => r.url.endsWith(APIEndpoint.UPDATE_CATEGORY_DETAILS));
