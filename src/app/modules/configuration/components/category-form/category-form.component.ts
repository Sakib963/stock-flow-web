...
/** Every control on this form, so the aria helpers cannot be asked about a field that is not here. */
type FormField = 'name' | 'category_code' | 'description' | 'status';
import { Category, CategoryField, CategoryPayload, CategoryStatus } from '@app/core/models/category.model';
...
@Component({
    ...
    imports: [ReactiveFormsModule, NgIcon, NzButtonModule, NzFormModule, NzInputModule, NzSelectModule, TranslatePipe],
...
export class CategoryFormComponent {
    ...
    readonly form = this._builder.group({
        name: ['', [Validators.required, Validators.maxLength(255)], [uniqueValue((value) => this._categories.isAvailable('name', value, this.editing()?.oid))]],
        category_code: ['', [Validators.required, Validators.maxLength(50)], [uniqueValue((value) => this._categories.isAvailable('category_code', value, this.editing()?.oid))]],
    ...
    /** Marks the field the server said collided, using the server's own words. */
    reject(field: CategoryField, message: string): void {
        const control = this.form.controls[field];
        control.setErrors({ ...(control.errors ?? {}), server: message });
        control.markAsTouched();
    }

    invalid(field: FormField): boolean | null {
        const control = this.form.controls[field];
    ...
    describedBy(field: FormField): string {
        return `category-${field.replace('_', '-')}-${this.invalid(field) ? 'error' : 'help'}`;
    }
