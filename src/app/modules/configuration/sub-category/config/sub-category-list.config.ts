...
export const SUB_CATEGORY_LIST: ListShellPageConfig = {
    ...
    table: {
        ...
        columns: [
            { key: 'category_code', label: 'configuration.subCategory.code', type: 'identifier', copy: true, width: 174, sortable: true, locked: true, pin: 'start' },
            { key: 'name', label: 'configuration.subCategory.name', type: 'name', sortable: true, width: 400, locked: true },
            { key: 'category_name', label: 'configuration.subCategory.category', type: 'text', copy: true, width: 174, sortable: true },
            { key: 'status', label: 'configuration.subCategory.status.label', type: 'status', width: 120, sortable: true, tones: SUB_CATEGORY_STATUS },
            { key: 'last_action_by', label: 'configuration.subCategory.lastTouched', type: 'user', width: 160, sortable: true, sortKey: 'last_action_on', name: 'last_action_by_name' },
            { key: 'last_action_on', label: 'configuration.subCategory.lastTouchedOn', type: 'date', format: 'date', width: 120, sortable: true, sortKey: 'last_action_on' },
            { key: 'created_on', label: 'configuration.subCategory.created', type: 'date', format: 'date', width: 120, sortable: true, hidden: true },
        ],
        layouts: [{ type: 'table' }],
        rowActions: [
